...
def pass_simple_function_as_parameter():
    ...
    def f(b, *args):
        print(f'{b}: {", ".join(args) if args else "no members specified"}')

    def g(f, b, *args):
        f(b, *args)

    g(f, 'The Beatles', *the_beatles)
    g(f, *the_beatles)
    g(f, 'The Beatles')
...
def return_function_with_args(*args):
    ...
    return empty if not args else tuple_of_args
...
# print(f())
f = return_function_with_args(23)
...
#%%
songs('Eleanor Rigby', 'She Said She Said', 'I\'m Only Sleeping')

#%%
f = a_very_simple_decorator(songs)
f('Eleanor Rigby', 'She Said She Said', 'I\'m Only Sleeping')
...
def band_details(f_to_decorate):
    ...
    def wrapper(*args, **kwargs):
        ...
        print(', '.join([a for a in args[1:]]) if args[1:] else 'no members specified')
        if kwargs:
            print(', '.join([str(k) + ': ' + str(v) for k, v in kwargs.items()]))
        else:
            print('no years active')
        return v
